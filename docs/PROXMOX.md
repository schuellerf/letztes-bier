...
5. **Bar “Erinnern”**: **`POST /api/custom/bar/remind-request`** (**`pb_hooks/bar_remind_request.pb.js`**) accepts JSON **`{"requestId":"<id>"}`** with **`users`** auth (`role = bar`). It reuses the same storage-staff push as a new pending request but with title prefix **Erinnerung ·** (rate limit **10s** per request id per PocketBase process; matches the bar UI cooldown). Requires the same **`PUSH_INTERNAL_TOKEN`** / relay setup as other pushes.
